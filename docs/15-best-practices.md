...
| Запустить инструмент вне проекта (изолированно) | `uvx ruff check .` |
| Запустить инструмент конкретной версии (изолированно) | `uvx ruff@0.15.0 check .` |

!!! note "`uvx` vs `uv run`"
    `uvx` создает отдельное временное окружение - инструмент не видит
    зависимости проекта и не привязан к версии из `uv.lock`. Это удобно
    для разового запуска инструмента, не входящего в зависимости проекта.

    Если инструмент добавлен в dev-зависимости, запускайте его через `uv run` -
    это гарантирует версию из `uv.lock` и доступ к зависимостям проекта.
    В CI используйте `uv run --frozen`.
...
| Старая команда | Эквивалент uv | Примечание |
| -------------- | ------------- | ---------- |
| `pipx run ruff check .` | `uvx ruff check .` | Разовый запуск вне проекта |
