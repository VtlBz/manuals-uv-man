...
Если lock-файл поврежден или содержит конфликт после merge - удалить и пересоздать:
...
**Решение:** ослабить версионные ограничения или обновить конфликтующие пакеты.
...
- `ModuleNotFoundError` при `uv run`, хотя пакет есть в lock-файле.
...
| ОС | Путь |
| -- | ---- |
| Linux / WSL 2 | `~/.cache/uv` (`$XDG_CACHE_HOME/uv`) |
...
!!! warning "Рост кеша"
    При активном использовании `uvx`, MCP-серверов и множества
    проектов кеш может вырасти до десятков гигабайт.
...
Удаляет только **неиспользуемые** записи: кеш от старых версий `uv`, wheels,
которые не нужны текущим проектам. Безопасно запускать в любое время.
...
После полной очистки следующая `uv sync` скачает все заново.
Использовать при подозрении на повреждение кеша.
...
Проверяет актуальность пакетов на PyPI без удаления кеша.
...
для загрузки пакетов. Флаг `--frozen` запрещает обновление lock-файла,
