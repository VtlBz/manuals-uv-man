...
!!! note "Установка через pip"
    Установка `uv` через `pip` работает, но требует уже установленного Python
    и не поддерживает `uv self update`. Standalone-установщик рекомендуется,
    поскольку не зависит от Python в системе и поддерживает самообновление.
...
!!! note "Даунгрейд версий"
    Даунгрейд на более старую версию не всегда безопасен: новая версия могла
    изменить формат кеша или lockfile.
