...
Параметр `package = false` указывает, что корневой проект не является пакетом
и не должен устанавливаться в окружение. Это типично для workspace root,
который служит только точкой конфигурации.
