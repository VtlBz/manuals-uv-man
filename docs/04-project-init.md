...
!!! info "Когда можно пропустить этот раздел"
    Этот раздел обязателен для нового (greenfield) проекта.
    Если проект уже существует, раздел можно пропустить и перейти в
    [Миграция с существующих инструментов](12-migration.md).
...
- По умолчанию, если Python 3.12 не найден, `uv` скачает managed
  ...
  `python-downloads` (подробнее - в разделе [Управление версиями Python](03-python-versions.md)).
...
`--no-dev`, `--group`, `--only-group`) - в разделе [Группы зависимостей](06-dependencies.md#группы-зависимостей).
...
Подробнее - в разделе [Опциональные зависимости](06-dependencies.md#опциональные-зависимости-projectoptional-dependencies).
...
(`--app`) секция не нужна.
Подробнее - в разделе [Сборка и публикация](11-publishing.md).
...
- `.venv` появится после первой команды, которой нужно окружение:
  `uv sync` или `uv run`.
  Подробно окружения разбираются в разделе [Виртуальные окружения](05-environments.md).
...
Подробнее - в разделе [Виртуальные окружения](05-environments.md).
...
pytest), процедура интеграции `uv` описана в разделе [Миграция с существующим pyproject.toml](12-migration.md#работа-с-существующим-pyprojecttoml).
